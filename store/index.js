    ...
    return new Vuex.Store({
        ...
        actions: {
            async REQUEST_LOGIN({ commit }, { id, password }) {
                ...
                const response = await requestMiddleware(localeApi.login, {
                    id,
                    password
                });
            ...
            SORT_TRANSLATE_LIST: function({ commit, state }, sortType) {
                let translateList = sortTranslateList(
                    state.currentTranslateList,
                    sortType,
                    state.currentProject.baseLang
                );
            ...
            async UPLOAD_FORM({ commit, state }, { projectName, elId }) {
                ...
                const response = await requestMiddleware(localeApi.uploadForm, {
                    projectName,
                    elId
                });

                if (response && response.code === "ok") {
                    const listRes = await requestMiddleware(
                        localeApi.getTranslateList,
                        state.currentProject.uuid
                    );
                    let translateList =
                        listRes && listRes.result ? listRes.result : [];
                    translateList = sortTranslateList(
                        translateList,
                        state.selectSortType,
                        state.currentProject.baseLang
                    );
                    commit("UPDATE_TRANSLATE_LIST", translateList);
                }
            ...
            async CREATE_USER({ commit }, { id, password }) {
                commit("UPDATE_LOADING_STATE", true);
                const response = await requestMiddleware(localeApi.createUser, {
                    id,
                    password
                });
                commit("UPDATE_LOADING_STATE", false);
            ...
            async ADD_TRANSLATE({ commit, state }, strData) {
                commit("UPDATE_LOADING_STATE", true);
                const response = await requestMiddleware(
                    localeApi.addTranslate,
                    { strData, projectData: state.currentProject }
                );
                if (response && response.code === "ok") {
            ...
            async REMOVE_TRANSLATE({ commit, state }, id) {
                commit("UPDATE_LOADING_STATE", true);
                const response = await requestMiddleware(
                    localeApi.removeTranslate,
                    { id, projectName: state.currentProject.name }
                );
                if (response && response.code === "ok") {
            ...
            async REMOVE_ALL_TRANSLATE({ commit, state }, { name, uuid }) {
                commit("UPDATE_LOADING_STATE", true);
                const response = await requestMiddleware(
                    localeApi.removeAllTranslateInProject,
                    { name, uuid }
                );
                if (response && response.code === "ok") {
        ...
        mutations: {
            ...
            UPDATE_TRANSLATE_TO_CURRENT: function(state, data) {
                let i,
                    len = state.currentTranslateList.length;
                for (i = 0; i < len; i += 1) {
            ...
            REMOVE_TRANSLATE_TO_CURRENT: function(state, strid) {
                ...
                for (i = 0; i < len; i += 1) {
                    if (
                        state.currentTranslateList[i] &&
                        state.currentTranslateList[i].uid === strid
                    ) {
