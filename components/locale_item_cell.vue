<template>
    ...
    >
        ...
        <td v-if="pSelectLang !== 'all'" width="28%" class="tr_kr">
            <span class="translate_txt" v-show="!isEdit">{{
                localeObj[pSelectLang]
            }}</span>
            <textarea
                v-show="isEdit"
                class="tr_kr"
                v-model="editLocaleObj[pSelectLang]"
                @keydown.enter="textEnterKey"
            ></textarea>
        </td>
        <td
            v-if="pSelectLang === 'all'"
            width="10%"
            class="tr_kr"
            v-for="(item, index) in projectLanguages"
        >
            <span class="translate_txt" v-show="!isEdit">{{
                localeObj[item]
            }}</span>
            <textarea
                v-show="isEdit"
                class="tr_kr"
                v-model="editLocaleObj[item]"
                @keydown.enter="textEnterKey"
            ></textarea>
        </td>
        <td width="17%">
            <button v-show="!isEdit" @click.stop="deleteTranslate()">
                삭제
            </button>
            <button v-show="!isEdit" @click.stop="addCopyTranslate()">
                복사추가
            </button>
        ...
        saveEdit: function() {
            this.$store
                ...
                .then(res => {
                    if (res) {
                        if (res.code === "ok") {
                            this.localeObj = JSON.parse(
                                JSON.stringify(this.editLocaleObj)
                            );
